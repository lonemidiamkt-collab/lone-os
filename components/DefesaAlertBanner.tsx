...
/** `onAbrir`: dentro do Tráfego, troca para a aba Defesa Ativa em vez de navegar (Leva 4). */
export default function DefesaAlertBanner({ onAbrir }: { onAbrir?: () => void } = {}) {
  ...
  const classe = `block w-full text-left rounded-xl border ${bgClass} p-3 hover:brightness-110 transition-all`;
  const conteudo = (
  ...
  return onAbrir
    ? <button type="button" onClick={onAbrir} className={classe}>{conteudo}</button>
    : <Link href="/traffic?aba=defesa" className={classe}>{conteudo}</Link>;
}
