        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-card border border-border rounded-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-border flex items-center justify-between">
              ...
              <button onClick={() => setShowForm(false)} className="text-muted-foreground hover:text-white"><X size={16} /></button>
            </div>
                  ...
                  {MEETING_TYPES.map((t) => (
                    <button key={t.value} onClick={() => {
                      ...
                      className={`p-2 rounded-lg border text-center text-xs transition-all ${
                        form.type === t.value ? "border-primary/50 bg-primary/10 text-white" : "border-border text-muted-foreground"
