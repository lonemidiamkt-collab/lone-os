...
export function useAnuncios({ periodo, clientesIds, selecionado }: {
  ...
  const aplicarItem = useCallback((item: ItemAnuncios) => {
    ...
    setDetalhe((d) => (d && d.clientId === item.clientId && item.campanhas ? item : d));
  }, []);
  ...
  // ── Atualizar agora ──────────────────────────────────────────────────────
  const atualizar = useCallback(async (clientId: string, opts?: { silencioso?: boolean }): Promise<Resultado | "erro"> => {
    ...
    return resultado;
  }, [periodo, marcar, aplicarItem, acompanhar]);
    ...
    const trabalhador = async () => {
      while (fila.length && vivo.current && !parouPorConexao) {
        ...
        for (let i = 0; i < 4 && res === "ocupado"; i++) {
          ...
          res = await atualizar(id, { silencioso: true });
        }
