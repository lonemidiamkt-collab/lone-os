...
function Numeros({ l }: { l: LinhaHoje }) {
  ...
  const saldo = n.statusConta
    ? <Numero rotulo="Conta" valor={n.statusConta} tom={COR.critical.texto} apoio="anúncios parados ou em risco" />
    : n.cartao
  ...
  return (
    <dl className="grid grid-cols-3 gap-3 lg:w-[23rem] lg:shrink-0">
      {saldo}
      <Numero
        rotulo="Gasto ontem"
  ...
  return (
    <li className="relative rounded-xl border border-border bg-card py-3 pl-4 pr-3">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        ...
        <div className="flex flex-wrap items-center gap-1.5 lg:w-[10.5rem] lg:shrink-0 lg:flex-col lg:items-stretch">
          ...
          {l.linkContaExterno ? (
            <a href={l.linkConta} target="_blank" rel="noopener noreferrer" className={BOTAO} title="Abrir no Gerenciador de Anúncios">
              <ExternalLink size={13} aria-hidden /> Abrir conta
            </a>
          ) : (
            <Link href={l.linkConta} className={BOTAO} title="Resultados do cliente no painel">
              <ExternalLink size={13} aria-hidden /> Abrir conta
            </Link>
          )}
          <button type="button" className={BOTAO} onClick={() => acoes.onPedirCriativo(l)}>
            <Palette size={13} aria-hidden /> Pedir criativo
          </button>
        </div>
      </div>
    </li>
