...
export const TEMPLATES_PADRAO: Templates = {
  ...
  decisor_contexto: T("diretriz",
    "Você chegou ao decisor ({decisor}). Cumprimente, apresente-se em uma frase ({agente}, da Lone Mídia) e explique por que entrou em contato: a Lone é especializada em construção civil, atende {empresas_atendidas}, com trabalho focado em geração de demanda, tráfego e estrutura comercial. Cite o que viu na {empresa} — SOMENTE se houver {gancho} em FATOS — e diga que achou que poderia fazer sentido mostrar o que estão aplicando em empresas parecidas. PARE AÍ: não proponha reunião, não pergunte se pode marcar, não termine com pergunta de venda. Espere a resposta.",
