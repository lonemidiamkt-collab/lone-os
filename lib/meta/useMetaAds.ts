...
export async function fetchCampaignInsights(
  ...
  // PAGINA todas as campanhas ACTIVE/PAUSED (antes parava em 100 → contas com >100
  // campanhas perdiam metade dos dados). Mantém em sync com insights-server.ts.
  const campaigns: any[] = [];
  ...
  // insights-server.ts). ~500 chamadas simultâneas faziam a maioria das campanhas falhar.
  const out: any[] = [];
