...
/** O filtro que o Início aplica aos itens dele: tira o alerta de tráfego visto (e ainda no prazo). */
export function ocultarVistosNoInicio(mapa: MapaVistos, agora: Date = new Date()) {
  return (i: { problema: string; severidade: NivelAlerta; cliente: { id: string } | null }): boolean => {
    const tipo = tipoDoProblemaInicio(i.problema);
    return !!tipo && estaVisto(mapa, i.cliente?.id, tipo, i.severidade, agora);
  };
}
