// lib/cs/roteiro-pdf.ts — HTML branded (Lone Mídia) de um roteiro de anúncio, p/ virar PDF
// via browserless (htmlToPdf). Usado no envio proativo de segunda pro social gravar com o cliente.
...
/** Monta o HTML A4 do roteiro. logoDataUri = data:image/png;base64,... (ou URL). */
export function roteiroPdfHtml(cliente: string, roteiro: Roteiro, logoDataUri: string, dataLabel: string): string {
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .head { display:flex; align-items:center; justify-content:space-between; border-bottom:3px solid ${BRAND}; padding-bottom:18px; margin-bottom:26px; }
  .head img { height:40px; }
  .head .meta { text-align:right; color:#6b7280; font-size:11px; line-height:1.5; }
  h1 { font-size:22px; color:${BRAND}; margin-bottom:4px; }
  .sub { color:#374151; font-size:13px; margin-bottom:22px; }
  ...
  .foot { margin-top:30px; border-top:1px solid #e5e7eb; padding-top:12px; color:#9ca3af; font-size:10px; text-align:center; }
    ...
    <div class="foot">Lone Mídia · Roteiro gerado pelo Agente Criativo · revise antes de gravar</div>
  </body></html>`;
}
