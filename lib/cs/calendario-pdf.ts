// lib/cs/calendario-pdf.ts — HTML branded do calendário estratégico (padrão Lone Mídia). Usado
// pela rota /api/cs/calendario/pdf (download) E pelo comando de WhatsApp (envia PDF no grupo).
...
export function calendarioPdfHtml(opts: { cliente: string; nicho?: string; periodo: string; modo: "mes" | "semana" | string; pecas: PecaFinal[] }): string {
  const cliente = esc(opts.cliente || "Cliente");
  const nicho = esc(opts.nicho || "");
  const periodo = esc(opts.periodo || "");
  const modo = opts.modo === "mes" ? "Mês" : "Semana";
