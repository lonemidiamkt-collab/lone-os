...
// Central de Comunicação — chats POR CLIENTE (o Chat Global foi removido a pedido do Roberto:
// o time não usava e atrapalhava. Cada mensagem de cliente vira histórico operacional dele).
...
import {
  MessageCircle, Search, Send, Activity,
  ChevronRight, Pin, Reply, X,
} from "lucide-react";
...
export default function CommunicationsPage() {
  ...
  const [mode, setMode] = useState<string>(""); // clientId selecionado ("" = nenhum)
  ...
  const messages = mode ? (clientChats[mode] ?? []) : [];
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <Header title="Central de Comunicação" subtitle="Chats por cliente" />

      <div className="flex flex-1 overflow-hidden">
        {/* ── Left sidebar: client chat list ─────────────────────────────── */}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {lastMsg && <p className="text-xs text-muted-foreground/50 mt-0.5 truncate">{lastMsg.text}</p>}
                  </div>
                  {msgs.length > 0 && <span className="text-xs text-muted-foreground/50 shrink-0">{msgs.length}</span>}
                </button>
              );
            })}
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {!selectedClient ? (
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground/50 gap-3 p-6 text-center">
              ...
              <p className="text-sm">Selecione um cliente à esquerda para ver e enviar mensagens da equipe sobre ele.</p>
            </div>
          ) : (
            <>
              {/* Chat header */}
              <div className="h-14 border-b border-border px-5 flex items-center gap-3 shrink-0">
                ...
                <Link href={`/clients/${selectedClient.id}`} className="btn-ghost text-xs flex items-center gap-1.5">
                  <Activity size={13} /> Histórico
                </Link>
                <button
                  onClick={() => { setShowMessageSearch(!showMessageSearch); setMessageSearch(""); }}
                  className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                    showMessageSearch ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                  }`}
              ...
              {showMessageSearch && (
                <div className="px-5 py-2 border-b border-border flex items-center gap-2 bg-muted/30">
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  {messageSearch && <span className="text-[10px] text-muted-foreground">{displayMessages.length} resultado(s)</span>}
                  <button onClick={() => { setShowMessageSearch(false); setMessageSearch(""); }} className="text-muted-foreground hover:text-foreground">
                    <X size={12} />
                  </button>
                </div>
              )}
              ...
              {channelPinned.length > 0 && (
                <div className="px-5 py-2 border-b border-primary/10 bg-primary/[0.02]">
                  <div className="flex items-center gap-1.5 mb-1">
                    <Pin size={10} className="text-primary" />
                    <span className="text-[10px] text-primary font-medium">{channelPinned.length} fixada(s)</span>
                  </div>
              ...
              <div className="flex-1 overflow-auto p-5 space-y-4">
                {displayMessages.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full text-muted-foreground/50 gap-3">
                    ...
                    <p className="text-sm">{messageSearch ? "Nenhuma mensagem encontrada." : "Nenhuma mensagem neste chat. Seja o primeiro!"}</p>
                  </div>
                )}
                {(messageSearch ? displayMessages : messages).map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex flex-col group ${isMe ? "items-end" : "items-start"}`}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {isPinned && <Pin size={9} className="text-primary" />}
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => togglePin(msg.id)} title={isPinned ? "Desafixar" : "Fixar"} className="w-5 h-5 rounded flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all">
                            <Pin size={9} />
                          </button>
                          <button onClick={() => setReplyTo({ id: msg.id, user: msg.user, text: msg.text })} title="Responder" className="w-5 h-5 rounded flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all">
                            <Reply size={9} />
                          </button>
                        </div>
                      </div>
                      {replyPart && (
                        <div className={`text-[10px] text-muted-foreground mb-0.5 px-3 py-1 rounded-lg bg-muted/40 border-l-2 border-primary/30 ${isMe ? "mr-1" : "ml-1"}`}>
                          {replyPart.replace("↩️ ", "")}
                        </div>
                      )}
                      <div className={`rounded-2xl px-4 py-2.5 text-sm max-w-[70%] whitespace-pre-wrap ${
                        isMe ? "bg-primary/30 text-primary rounded-tr-sm" : "bg-card border border-border text-foreground rounded-tl-sm"
                      }`}>
                        {mainText}
                      </div>
                    </div>
              ...
              {/* Reply-to */}
              {replyTo && (
                <div className="px-5 py-2 border-t border-primary/10 bg-primary/[0.02] flex items-center gap-2">
                  <Reply size={12} className="text-primary shrink-0" />
                  <div className="flex-1 min-w-0">
                    <span className="text-[10px] text-primary font-medium">Respondendo a {replyTo.user}</span>
              ...
              <div className="p-4 border-t border-border">
                <div className="flex gap-3 items-center">
                  ...
                  <input
                    ...
                    placeholder={replyTo ? `Responder a ${replyTo.user}...` : `Mensagem sobre ${selectedClient.name}...`}
                  ...
                  />
                  <button onClick={handleSend} disabled={!input.trim()} className="btn-primary px-4 py-2.5 flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed">
                    <Send size={15} />
                  </button>
                </div>
          ...
          <div className="hidden lg:flex w-60 border-l border-border flex-col shrink-0">
            <div className="p-4 border-b border-border">
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex justify-between"><span className="text-muted-foreground">Status</span><span className={`badge border ${getStatusColor(selectedClient.status)}`}>{getStatusLabel(selectedClient.status)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Social</span><span className="text-foreground">{selectedClient.assignedSocial}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Tráfego</span><span className="text-foreground">{selectedClient.assignedTraffic}</span></div>
              </div>
            </div>
            <div className="p-4">
              <Link href={`/clients/${selectedClient.id}`} className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-primary/10 text-primary text-xs font-medium border border-primary/20 hover:bg-primary/20 transition-all">
