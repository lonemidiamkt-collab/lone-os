...
// A agenda da agência (posts, tarefas, rotina de tráfego, reuniões e lembretes). Mora aqui e aparece
// como a vista "Agenda" do Meu Trabalho (/my-work?view=agenda); /calendar redireciona pra lá.
// `embutido`: sem o título próprio — quem mostra o título é o Meu Trabalho.
export default function Agenda({ embutido = false }: { embutido?: boolean }) {
  ...
  const allEvents = useMemo<CalendarEvent[]>(() => {
    ...
    reunioes.forEach((r) => {
      ...
      const cara = r.estado === "realizada" ? { sufixo: " · realizada", cor: "bg-lone-success" }
        : r.estado === "no_show" ? { sufixo: " · não compareceu", cor: "bg-lone-warning" }
        : r.estado === "cancelada" ? { sufixo: " · cancelada", cor: "bg-muted-foreground" }
        : { sufixo: "", cor: TYPE_COLORS.meeting };
      events.push({
        id: `reu-${r.id}`,
        type: "meeting",
        title: `Reunião — ${r.cliente}${cara.sufixo}`,
  ...
  return (
    <div className={embutido ? "space-y-6" : "p-6 space-y-6 animate-fade-in"}>
      {/* Header */}
      <div className={`flex items-center gap-3 ${embutido ? "justify-end" : "justify-between"}`}>
        {!embutido && (
          <div>
            <h1 className="text-lone-h1 tracking-tight text-foreground flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={embutido
              // No Meu Trabalho o botão principal é o "Nova tarefa" do cabeçalho; aqui fica secundário.
              ? "flex items-center gap-1.5 px-3 py-2 rounded-xl border border-border bg-card text-foreground text-xs font-medium hover:bg-accent transition-all"
              : "flex items-center gap-1.5 px-3 py-2 rounded-xl bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/80 transition-all"}
          >
            <Plus size={14} /> {embutido ? "Criar na agenda" : "Criar"}
          </button>
                      ...
                      <div className="flex items-center justify-between mb-0.5">
                        {(() => {
                          ...
                          const dayColor = hasDeadline
                            ...
                            : isNational
                            ? "text-lone-warning font-semibold"
                            : isState
                            ? "text-lone-warning font-semibold"
                            : isMunicipal
                      ...
                      <div className="flex flex-col gap-0.5 flex-1">
                        {nonReminderEvents.slice(0, 2).map((e) => {
                          ...
                          return (
                            ...
                            >
                              {isTaskDeadline && <AlertTriangle size={10} className="shrink-0" aria-label="Prazo" />}
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-xs font-medium truncate ${
                            ...
                            isTaskDeadline ? "text-destructive font-semibold" : "text-foreground"
                          }`}>
                            {e.title}
                          </span>
                        </div>
                        ...
                        {isTaskDeadline && (
                          <div className="ml-[18px] mt-1">
                            <span className="text-[9px] px-1.5 py-0.5 rounded bg-destructive/10 text-destructive font-semibold border border-destructive/20">
                              PRAZO FINAL
                            </span>
                          </div>
                        )}
                      ...
                      {e.type === "meeting" && (() => {
                        ...
                        if (reu && reu.estado !== "agendada") {
                          const rot = reu.estado === "realizada" ? "Realizada"
                            : reu.estado === "no_show" ? "Cliente não compareceu" : "Cancelada";
                  ...
                  return (
                    ...
                    >
                      <span className={`text-[10px] w-10 shrink-0 text-right font-medium ${
                        isTaskToday ? "text-destructive font-semibold" : isTomorrow ? "text-lone-warning" : "text-muted-foreground"
                      }`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`text-xs truncate block ${isTaskToday ? "text-destructive font-semibold" : "text-foreground"}`}>
                          {task.title}
                        </span>
        ...
        <div className="p-6 space-y-5">
          ...
          {isDeadlinePassed && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/20">
              ...
              <span className="text-xs text-destructive font-semibold">Prazo vencido — entrega era {task.dueDate}</span>
            </div>
          )}
