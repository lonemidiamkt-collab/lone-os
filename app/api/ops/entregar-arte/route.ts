...
// Só depois do commit: aviso ao social; a revisão por IA é disparada pelo cliente (best-effort, como antes).
export async function POST(req: NextRequest) {
  ...
  return comExecucao({ origem: "ops:entregar-arte", ator: gate.user.email, papel: gate.papel }, async () => {
    ...
    const quem = (tm?.name as string) || gate.user.email;
    ...
    const estado = data as { repetida?: boolean; delivery: { version: number; urls: string[] }; card: { id: string }; demanda: { id: string } | null };
    if (!estado.repetida) {
